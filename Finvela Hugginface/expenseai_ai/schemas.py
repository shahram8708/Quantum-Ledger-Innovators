...
class HeaderConfidence(BaseModel):
    """Confidence scores for each header field returned by the vision model."""
...
class LineItemModel(BaseModel):
    """Validates a single line item entry from the vision model."""
...
class ParseResult(BaseModel):
    ...
    @classmethod
    def from_model_payload(cls, raw: dict) -> "ParseResult":
        """Validate and normalize a raw vision-language payload into a `ParseResult`."""
        try:
            return cls.model_validate(raw)
        except ValidationError as exc:  # pragma: no cover - defensive validation path
            raise ValueError(f"Invalid model payload: {exc}") from exc
